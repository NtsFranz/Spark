...
{#if $frame}
	<div id="main_banner">
		<div id="entire_clock_area">
			...
			<div class="center_clock_area">
				...
				<div class="possession orange">
					<div id="possession_orange"
						 class="{$frame['possession'][0] === 1 && $frame['game_status'] === 'playing' ? 'active' : ''}"></div>
				</div>
				<div class="possession blue">
					<div id="possession_blue"
						 class="{$frame['possession'][0] === 0 && $frame['game_status'] === 'playing' ? 'active' : ''}"></div>
				</div>
				<div id="round_scores_combined" class="round_scores"></div>
			</div>
			<div id="points_orange" class="points orange">{$frame['orange_points']}</div>

			<div class="moving_part_container orange">
				<div class="moving_part orange {$overlay_config && $overlay_config['visibility']['show_team_names'] ? 'visible' : ''}">
					<div class="team_logos_collapsible orange"
						 class:visible={$overlay_config && $overlay_config['visibility']['show_team_logos']}>
						<div class="points secondary orange left"></div>
						<div id="team_logo_background_orange" class="team_logo_background orange"
							 style="background-image: url('{$overlay_config ? $overlay_config['teams'][1]['team_logo'] : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII='}');"></div>
						<div class="points secondary orange right"></div>
					</div>
			...
			<div class="moving_part_container blue">
				<div id="blue_goal_banner" class="moving_part blue"
					...
					<div class="team_logos_collapsible blue {$overlay_config && $overlay_config['visibility']['show_team_logos'] ? 'visible' : ''}">
						<div class="points secondary blue right"></div>
						<div id="team_logo_background_blue" class="team_logo_background blue"
							 style="background-image: url('{$overlay_config ? $overlay_config['teams'][0]['team_logo'] : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII='}');"></div>
						<div class="points secondary blue left"></div>
					</div>
				</div>
			</div>
		...
		{#if $overlay_config}
			<div class="round-wins-bottom">
				{#each Array($overlay_config['round_scores']['round_count']) as _, i}
					{#if $overlay_config['round_scores']['round_scores_orange'].length - 1 > i}
						<!-- Finished rounds -->
						<div>
							<div class="{$overlay_config['round_scores']['round_scores_orange'][i] > $overlay_config['round_scores']['round_scores_blue'][i] ? 'orange' : 'blue'}"></div>
						</div>
					...
					{:else if $overlay_config['round_scores']['round_scores_orange'].length - 1 === i}
						...
						<div class="unfinished">
							<div class="orange"
								 style="flex-grow:{$overlay_config['round_scores']['round_scores_orange'][i]}"></div>
							<div class="blue"
								 style="flex-grow:{$overlay_config['round_scores']['round_scores_blue'][i]}"></div>
						</div>
					{:else}
						<!-- Future rounds -->
						<div></div>
			...
			<div style="position:absolute;right: 8.75em;top: 4.1em; z-index: -90;">
				<DiagonalDropdown
			...
			<div style="position:absolute;left: 8.4em;top: 4.1em; z-index: -100;">
				<DiagonalDropdown
					...
					<div class="speed"
						...
						<span style="font-size: 1em;">{$mathematical_time < 0 ? 'NO TIME' : $mathematical_time.toFixed(0) + ' s'}</span>
					</div>
					<div style="position:absolute; bottom: 0;left: 2.8em; font-size: .7em; font-weight: 900; color: #fff7;">
						MATH. TIME
					</div>
